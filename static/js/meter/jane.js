...
$("#val").html(jane_info[0][0]);
$("#lval").html("Time: " + jane_info[0][1]);
$("#pval").html(jane_info[0][3]);

jane_slider.on('input propertychange', function(){
    console.log(this.value);
    console.log(jane_info[(this.value / 10).toFixed(0)-1][2]);
    console.log('url("../img/bgimg/' + jane_info[(this.value / 10).toFixed(0)-1][2] + '.png")');
    $("header.masthead").css('background', 'linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url("static/img/bgimg/' + jane_info[(this.value / 10).toFixed(0)-1][2] + '.png") center center no-repeat');
    console.log(jane_info[(this.value / 10).toFixed(0)-1][0]);
    $("#val").html(jane_info[(this.value / 10).toFixed(0)-1][0]);
    $("#lval").html("Time: " + jane_info[(this.value / 10).toFixed(0)-1][1]);
    $("#pval").html(jane_info[(this.value / 10).toFixed(0)-1][3]);
});